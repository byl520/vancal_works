    var login11=getCookie("login"); //用户id
    var src2=window.location;
    var reg=/^\?id=(.+)$/
    var goodId = reg.exec(src2.search)[1];//获取到商品id
    // console.log(login11)
     getData();
	async function getData() {
		var res = await pAjax({
			url: '../servers/car2.php',
			data: {
                'goodId':goodId,
				'login1': login11
			},
...
function loadGoods(data) {
		var goodsHtml = '<div class="goods-item">' +
			...
			'<img class="goods-image" src="' + data.goods_pics + '" style="width: 100px; height: 100px;" />' +
			'<span id="goods-info">' +
			data.goods_name +
			'</span>' +
			'</div>' +
			'<div class="col-md-3 car-goods-info goods-params">' + data.goods_size + '</div>' +
			'<div class="col-md-1 car-goods-info goods-price"><span>￥</span><span class="single-price">' + data.goods_price + '</span></div>' +
			'<div class="col-md-1 car-goods-info goods-counts">' +
			'<div class="input-group">' +
			'<div class="input-group-btn">' +
			'<button type="button" class="btn btn-default car-decrease">-</button>' +
			'</div>' +
			'<input type="text" class="form-control goods-count" value="' + data.goods_count + '">' +
			'<div class="input-group-btn">' +
			'<button type="button" class="btn btn-default car-add">+</button>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'<div class="col-md-1 car-goods-info goods-money-count"><span>￥</span><span class="single-total">' + data.goods_price + '</span></div>' +
